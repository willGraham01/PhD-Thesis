...
\begin{tikzpicture}[]
	...
	% period cell size
	\draw[<->] (0,6.2) -- (6,6.2); \node[anchor=south] at (3,6.2) {$1$};
	% inclusion size
	\draw[<->] (1,0.875) -- (5.0,0.875); \node[anchor=north] at (3.0,0.875) {$\delta$};
	% draw example material params: go for soft inclusion (a(x)=a) inside stiff matrix (a(x)=1)
	\node[anchor=center, align=center] at (3,3) {$\epsilon_r(\tilde{x})= \varepsilon = \frac{\epsilon^{\mathrm{inc}}}{\epsilon^{\mathrm{bulk}}}$ \\ $\mu_r(\tilde{x})= \mu = \frac{\mu^{\mathrm{inc}}}{\mu^{\mathrm{bulk}}}$};
	\node[anchor=north, align=center] at (1.5,5.5) {$\epsilon_r(\tilde{x})=1$ \\ $\mu_r(\tilde{x})=1$};
