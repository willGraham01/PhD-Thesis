...
However by examining the eigenvalues in this band as functions of $\qm$, as in figure \ref{fig:FDM_2022-01-28-09-33_AllBands}, it can be noticed that these extreme eigenvalues occur at quasi-momentum values where one of the components is either 0 (for the minimum values) or $-\pi$ (for the maximum values). \tstk{maybe move this to its own appendix as it's quite clunky, and then we can also throw in the alpha=1 ones too}
\begin{figure}[b!]
	\centering
...
We additionally note that the Dirichlet eigenvalues $\omega_{n,m}^2=\bracs{n^2+m^2}\pi^2$ remain, as expected, part of the spectrum \tstk{in fact, are bands shrinking TO these values as alpha increases? Try a run with big alpha to check!}.
