...
Analogous boundary conditions (and equations in the bulk) are observed in the study \tstk{Kirill-Evans} for a similar material setup, but considering the effective problem for the equations of (linearised) elasticity as the period cell size shrinks to zero.
...
The material considered in \tstk{Kirill-evans} did not possess geometric contrast (between the vertex and edge volume) however, so there is no analogous equation to \eqref{eq:SI-VertexCondition} appears in the effective problem.
