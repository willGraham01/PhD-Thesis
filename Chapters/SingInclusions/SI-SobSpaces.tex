...
\begin{lemma} \label{lem:SI-SmoothGradZero}
	...
	Then the function
	\begin{align*}
		\tilde{g} &= \begin{cases} 0 & x\in\ddom\setminus I_{jk} \\ \phi n_{jk} & x\in I_{jk} \in\gradZero{\ddom}{\lcompMes}. \end{cases}
	\end{align*}
\end{lemma}
\begin{proof}
	...
	\tstk{this is where we'd need that $\phi$ and it's gradient are bounded in $\ltwo{\ddom}{\lcompMes}$, which for the gradient isn't assured because of the aforementioned boundary issues. The statement of this lemma can be amended to provide a similar result regardless.}
...
Lemma \ref{lem:SI-GradZeroEdgeToComposite} essentially proves that we can ``lift" a gradient of zero on an edge up to a gradient of zero with respect to the composite measure $\lcompMes$.
...
Now we need an extension lemma so that we can move from $\lcompMes$ to $\compMes$...
