...
These arguments are the focus of \tstk{appendix section}, however we again provide the reader with a summary of the key properties of the tangential curl so that the manipulations in section \tstk{derivation section} can be followed, and to provide some familiarity with the objects that are discussed in \tstk{discussion session where everything is broken}.
...
\begin{enumerate}[(i)]
	...
	\item Along the edge $I_{jk}$, only the ``in-plane" components of $u$ are relevant, to this end we set $U^{(jk)} = R_{jk}\begin{pmatrix} u^{(jk)}_1 \\ u^{(jk)}_2 \end{pmatrix}$.
