...
Imposing that the field $u$ be divergence-free also serves a mathematical purpose; the kernel of the (matrix) operator $\mathcal{M}$ in \eqref{eq:Intro-MaxwellInMatrxForm} is infinite-dimensional, being the space of (pairs of) potential fields.
After ``squaring" $\mathcal{M}$ to obtain the curl-of-the-curl ``operator" (with non-negative spectrum), we still have this infinite-dimensional kernel and thus the zero eigenvalue has infinite multiplicity.
The zero eigenvalue, which is now the least eigenvalue, having infinite multiplicity makes numerical approaches that are justified through use of the min-max principle redundant --- with the least eigenvalue having an infinite-dimensional eigenspace, one can never compute non-zero eigenvalues.
Including the divergence-free condition in the definition of the domain of the Maxwell operator removes potential fields from the domain of our operator, and consequentially removes the eigenvalue of infinite multiplicity at zero.
The divergence-free condition also adds extra constraints to the fields, and the analysis of section \ref{sec:CC-CurlAnalysis} indicates that we require an analogue of this divergence-free condition to constrain the behaviour of vector fields with $\dddmes$-tangential curls near the vertices.
