...
We find ourselves presented with several reasonable definitions for the notion of a divergence free vector field with respect to our singular measures, which we will discuss in turn below.
However, we will see that the differences between definitions \ref{def:DivFree-AllGradients} and \ref{def:DivFree-TangGradients} is essentially nullified by the result of \tstk{derivation section}, whilst the ``strong" definition \tstk{definition ref} fails to get off the ground.
...
Utilising this analogue with the classical divergence, we can adopt the following definition for our analogue of divergence-free fields:
...
This notion of divergence-free allows for a rather quick characterisation of divergence-free vector fields.
...
We can take this as an assurance that our vector field $u$ is not inducing any flux or flow in the direction normal to $I_{jk}$.
...
This interpretation does raise the question as to whether we can define \emph{the divergence} of a vector field with respect to our singular measures, akin to how we defined gradients and curls in section \tstk{prelims section} --- we come to this later.
At present, we instead ask the question about whether being divergence-free should necessitate that a field be orthogonal to gradients of zero.
...
There is thus a case for proposing a weaker definition of a divergence-free, which for ease of language we will refer to as \emph{tangentially divergence-free}.
...
This brings us closer to the idea of defining a ``tangential divergence" in a similar manner to that of gradients and curls in section \tstk{ref!}.
...
\begin{definition}[$\kt$-Divergence (with respect to $\dddmes$)] \label{def:dddmesDivergence}
	...
	\end{align*}
	we say that $u$ admits (or possesses) a $\kt$-divergence with respect to $\dddmes$.
	Then we call $d:=\ktgrad_{\dddmes}\cdot u$ the $\kt$-divergence with respect to $\dddmes$ of the vector field $u$.
	If $\ktgrad_{\dddmes}\cdot u=0$, then $u$ is divergence free.
\end{definition}
Clearly we could produce a similar definition for any Borel measure $\rho$ on $\ddom$.
The notions of $\kt$-tangentially divergence free (definition \ref{def:DivFree-TangGradients}) and $\kt$-divergence free as in \ref{def:dddmesDivergence} coincide, and that the $\kt$-divergence of a field $u$ is unique --- if there are two such divergences, the difference integrated against all $\phi\in\psmooth{\ddom}$ must be zero, from which we can deduce the difference is zero almost everywhere.
And finally, we can conclude that $\kt$-divergence has the same form as the function $F$ in \eqref{eq:ktDivergence-Guess}, up to the position of the constant $\alpha_j$.
\begin{cory}
	Let $u\in\pltwo{\ddom}{\dddmes}^3$, and define (for each edge $I_{jk}$)
	\begin{align*}
		U^{(jk)} = R_{jk}\begin{pmatrix} u_1^{(jk)} \\ u_2^{(jk)} \end{pmatrix}, 
...
We can ask whether it would be possible to define the tangential divergence through an approximation via smooth functions in a manner akin to tangential gradients and curls.
...
\end{align*}
Similar arguments to those in section \tstk{ref} would inform us that $\mathcal{D}_{\ddom,\md\rho}\bracs{0}$ does not depend on the value of $\kt$, and is a closed, linear subspace of $\pltwo{\ddom}{\rho}$.
...
\end{proof}
Further to this, it is not difficult to prove an analogy of the extension lemmas \tstk{gradient and curl of zero extension lemmas} for these divergences of zero.
Neither is it difficult to show that $\mathcal{D}_{\ddom,\md\massMes}\bracs{0}=\pltwo{\ddom}{\massMes}$, and consequentially that an analogue of \tstk{curl and gradient of zero char theorems} holds, so every tangential divergence is zero!
