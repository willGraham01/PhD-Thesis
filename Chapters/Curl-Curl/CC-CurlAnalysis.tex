...
Further to this, we know from theorem \tstk{gradient characterisation} that functions in this space are precisely those that can be constructed from edge functions, provided these edge functions are continuous at the vertices.
...
Proposition \ref{prop:TC-3rdComponentIFF} also allows us to invoke theorem \tstk{gradient characterisation theorem} to conclude that the component $u_3$ is continuous at the vertices of $\graph$.
...
However we are lacking continuity (and even weak differentiability) of $u_1$ and $u_2$ at the vertices, so cannot sum edge-functions to obtain a function on the whole graph in a manner akin to \tstk{gradient case}.
The best we can do (utilising only knowledge of curls) comes when we have a vector field whose first and second components have support contained in the interior of an edge, analogous to \tstk{tang grad extension lemma}.
\begin{prop}
...
\tstk{but does my $L^1$-to-subsequence trick save us here though? Don't think so, as we still need to link up these functions somehow. Only divergence-free will give us that...}
