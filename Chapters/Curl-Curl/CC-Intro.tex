...
%\footnote{It is worth remarking that $\psmooth{\ddom}$ is the image under the Gelfand transform of $\csmooth{\reals^2}$ (see section \ref{sec:TP-GelfandTransform}). In lieu of how our spaces $\ktcurlSob{\ddom}{\dddmes}$ are then constructed from such functions, which is closure of pairs of smooth functions and their curls, the domain of $c_{\qm}$ consists of the product of two copies of $\ktgradSob{\ddom}{\dddmes}$. There are no distinct ``maximal" and ``minimal" spaces one typically sees when working with the curl operator in $\reals^3$ \cite{birman1987l2}.}
