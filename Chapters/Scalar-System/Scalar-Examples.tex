...
\begin{figure}[t!]
	...
	\end{subfigure}
	\caption{\label{fig:Diagram_1DExample} The graphs $\hat{\graph}$, $\graph$, and $\graph^*$.}
\end{figure} \newline
...
\begin{figure}[b!]
	...
	\end{subfigure}
	\caption{\label{fig:Diagram_1DAngledEdgeExample} The graphs $\graph$, $\graph_{\mathcal{Q}}$, and $\graph^*$.}
\end{figure}
...
Zeros of $H^{(2)}$ occur at $\omega= 2n\pi, \frac{n\pi}{a}, \frac{n\pi}{b}$, but examining the limit \eqref{eq:EigenvalueBranchLimit} reveals that if $H^{(2)}\bracs{\omega_0^2}=0$, $\omega_0$ is part of the spectrum only when there exists a $\qm_0\in\left[-\pi,\pi\right)$ such that $\Xi\bracs{\omega_0}=\cos\theta_0+\recip{2}$.
...
For a given $\qm$, equation \eqref{eq:ExampleThickVertexSolutionReduced} can be solved for (a solution) $\omega=\omega_0$\footnote{Conversely of course, we could choose an eigenvalue $\omega$ and compute the corresponding quasi-momentum for which $\omega\in\sigma_{\qm}$.}.
...
Some examples of the result of this process are plotted in figure \ref{fig:CrossInPlane-EdgePlot}, one should note that the expected continuity at the central vertex holds, whilst their incoming derivatives adhere to the (non-standard) Kirchoff condition. \tstk{check imaginary part - red to orange doesn't look continuous???}
\begin{figure}[h!]
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
...
\end{figure}
\tstk{should we define these and briefly explain how the approximation was obtained?}
