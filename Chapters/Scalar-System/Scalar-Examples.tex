...
\tstk{example via numerical solve of M-matrix?!?!?!}
...
Also define \tstk{you define these everywhere! Maybe number one of the equations and give it a rest?}
\begin{align*}
	 s_a\bracs{\omega} = \sin\bracs{a\omega}, \quad 
	 c_a\bracs{\omega} = \cos\bracs{a\omega}, \quad 
...
We can determine $w$ by computing the eigenvector of $\mathfrak{M}_{\qm}\bracs{\omega_0^2}$ corresponding to its zero eigenvalue.
...
To round off the analysis, quantities such as the integrated density of states (IDoS) and density of states (DoS) can also be estimated from \eqref{eq:ExampleThickVertexSolution}, shown in figure \ref{fig:CrossInPlane_ScalarDoS}, displaying the band-gap structure of the spectrum and demonstrating that the eigenvalues ``concentrate" at the center of each spectral band. \tstk{this appears to be in contrast to EG H-L, but they are coming from another perspective though. Also talk about where the bands start and end....}
\begin{figure}[t!]
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
