...
Justification lies in the observation that introducing explicit $\omega^2$-dependence should not affect the (structure of) the arguments in the supporting theory (see \cite[page 1846]{cherednichenko2018effective}, concerning the results of \cite{ryzhov2009weyl} \tstk{check with Kirill - are there other works that are more directly applicable that could be adapted?}), and consequentially similar conclusions can be drawn with this alteration to $B$.
...
\begin{prop} \label{prop:MMatrixDetForm}
	Given a graph $\graph = \bracs{\vertSet, \edgeSet}$ containing no loops, there exists a function $F\bracs{\qm,\omega}$ that is analytic in both its arguments, such that
