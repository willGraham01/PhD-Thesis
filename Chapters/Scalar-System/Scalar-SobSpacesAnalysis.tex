...
This approach will also guide us when we come to examine the Sobolev spaces of curls in section \ref{sec:CC-CurlAnalysis}.
...
Given proposition \ref{prop:ZeroInvariantUnderQM-Wavenumber}, without loss of generality we can always take any approximating sequence $\phi_n$ for a gradient of zero $g$ to be such that $\grad\phi_n\rightarrow g$, as opposed to $\ktgrad\phi_n\rightarrow g$.
...
\begin{proof}
	...
	\begin{align*}
		...
		\grad\phi_n \lconv{\ltwo{\ddom}{\lambda_{jk}}^3} g_{jk}\hat{e}_{jk}.
	\end{align*}
	Therefore, $\grad\phi_n\cdot\hat{e}_{jk}\rightarrow g_{jk}$, and thus
	\begin{align*}
		\int_0^{\abs{I_{jk}}} \abs{\diff{\phi_n}{y}\bracs{r_{jk}(y)} - g_{jk}\bracs{r_{jk}(y)} }^2 \ \md y
		&= \integral{I_{jk}}{ \abs{\grad\phi_n\cdot\hat{e}_{jk} - g_{jk}}^2 }{\lambda_{jk}} \toInfty{n}, \\
	...
	Then we have that $\grad\phi_n = \bracs{\bracs{x-v_j}\cdot n_{jk}}\grad g + g\hat{n}_{jk}$, and notice that $\bracs{x-v_j}\cdot n_{jk}	=0$ when $x\in I_{jk}$.
	It is now clear that $\phi=0$ and $\grad\phi = g\hat{n}_{jk}$ on $I_{jk}$, so $g\hat{n}_{jk}\in\gradZero{\ddom}{\lambda_{jk}}$.
...
\begin{proof}
	...
	\begin{align*}
		...
		\norm{\grad\phi_n - g^{(jk)}}_{\ltwo{\ddom}{\lambda_{jk}}} &\leq \norm{\grad\phi_n - g}_{\ltwo{\ddom}{\ddmes}} \rightarrow 0,
	\end{align*}
...
\begin{proof}
	...
	Furthermore,
	\begin{align*}
		\norm{\grad\psi_l - g}_{\ltwo{\ddom}{\ddmes}^2}^2
		&= \norm{\chi_{jk}^n\grad\phi_l + \phi_l\grad\chi_{jk}^n - g_{jk}}_{\ltwo{\ddom}{\lambda_{jk}}^2}^2 \\
		&\leq 2\norm{\phi_l\grad\chi_{jk}^n}_{\ltwo{\ddom}{\lambda_{jk}}^2}^2 + 2\norm{\chi_{jk}^n\grad\phi_l - g_{jk}}_{\ltwo{\ddom}{\lambda_{jk}}^2}^2 \\
		&\leq 2\sup_{I_{jk}}\abs{\grad\chi_{jk}^n}^2 \norm{\phi_l}_{\ltwo{\ddom}{\lambda_{jk}}^2}^2
		+ 2\norm{\grad\phi_l - g_{jk}}_{\ltwo{\ddom}{\lambda_{jk}}^2}^2 \\
		&\rightarrow 0 \toInfty{l}.
	\end{align*}
	Therefore, $\psi_l$ is an approximating sequence for $g$, and thus $g\in\gradZero{\ddom}{\ddmes}$, as required.
	
	Next, notice that $\psi_l\bracs{v_j} = \psi_l\bracs{v_k} = 0$, and $\grad\psi_l\bracs{v_j} = \grad\psi_l\bracs{v_k} = 0$ for every $l\in\naturals$.
	Therefore,
	\begin{align*}
		\norm{\psi_l}_{\ltwo{\ddom}{\massMes}} = 0 = \norm{\grad\psi_l}_{\ltwo{\ddom}{\massMes}^2},
	\end{align*}
	for every $l\in\naturals$, and so
	\begin{align*}
		\norm{\psi_l}_{\ltwo{\ddom}{\dddmes}} &= \norm{\psi_l}_{\ltwo{\ddom}{\ddmes}} \rightarrow 0, \\
		\norm{\grad\psi_l - \tilde{g}}_{\ltwo{\ddom}{\dddmes}^2} &= \norm{\grad\psi_l - g}_{\ltwo{\ddom}{\ddmes}^2}^2 \rightarrow 0,
	\end{align*}
...
\begin{align*}
	\norm{\grad\phi_l - g_{jk}}_{\ltwo{\ddom}{\lambda_{jk}}^2} \leq \norm{\grad\phi_l - g_{jk}}_{\ltwo{\ddom}{\lambda_{jk}}^2}
\end{align*} 
...
\begin{proof}
	...
	The function $\phi$ is smooth, has support contained in $B_d\bracs{v_j}$, and is such that $\grad\phi(x) = \grad\varphi_c\bracs{x-v_j}$.
	Clearly
	\begin{align*}
		\integral{\ddom}{\abs{\phi}^2}{\massMes} = 0, \quad
		\integral{\ddom}{\abs{\grad\phi - g_1^j}^2}{\massMes} = 0,
	\end{align*}
...
\begin{proof}
	...
	$\bracs{\Leftarrow}$ For the left-directed implication, it will be sufficient for us to show the result holds when $g_\ddmes=0$ and then for when $g_\massMes=0$, since we can use linearity of $\gradZero{\ddom}{\dddmes}$ to complete the proof.
	...
	For the case when $g_\ddmes=0$, we can again take without loss of generality $g_\massMes$ to be non-zero at exactly one vertex $v_j\in\vertSet$ (as again, we can then use linearity of $\gradZero{\ddom}{\dddmes}$).
	Let this be the case, so $g_\ddmes=0$ and $g_\massMes=0$ except at the vertex $v=\bracs{v_1,v_2}^\top\in\vertSet$, where $g_\massMes(v) =: g = \bracs{g_1,g_2,0}^\top$.
	...
	\begin{align*}
		...
		&= \integral{B_{2/n}(v)}{ \abs{\grad\phi_n}^2 }{\ddmes} + \alpha_v\abs{\grad\phi_n(v) - g}^2
