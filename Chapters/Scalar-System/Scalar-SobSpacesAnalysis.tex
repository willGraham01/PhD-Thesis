...
\end{align*} 
to hold.
It is also required so that we can use the function $\chi_{jk}^n$ to ensure that the support of our approximating sequence $\psi_l$ is only intersects $\graph$ on the edge $I_{jk}$, so we can utilise the properties of $\phi_l$.
...
We shall come to utilise the knowledge of tangential gradients in section \ref{sec:ScalarDerivation}, where knowing their explicit form is essential to us obtaining the quantum graph problem \eqref{eq:SingularWaveEqnQGProblem} from \eqref{eq:SingularScalarWaveEqn}.