...
We will come to utilise the knowledge of tangential gradients in section \tstk{ help section \ref{sec:}}, where knowing their functional form is essential to allow us to obtain a quantum graph problem from the problem \tstk{problem equation refs}
...
This also highlights the reason for defining the gradients of zero and tangential gradients as in \tstk{section ref} --- given a tangential gradient $\ktgrad_{\lambda_{jk}}u$, $\lambda_{jk}$ can reconstruct the function $u$ along the edge $I_{jk}$, but cannot determine what $u$ is doing across $I_{jk}$.
