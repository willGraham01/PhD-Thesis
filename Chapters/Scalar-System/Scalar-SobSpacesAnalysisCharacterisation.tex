...
As such, although the statement of the result in theorem \ref{thm:CharOfSobSpaces}(i) (in the absence of $\qm$ and $\wavenumber$) is made, the study does not provide an explicit proof and instead refers the reader to an analogous result \cite[lemma 6.1]{zhikov2002homogenization} in the aforementioned context of elasticity.
...
We can appeal to the Rellich-Kondrachov theorem (\cite[section 5.7, theorem 1]{evans2010partial}, \cite[chapter 6]{adams2003sobolev}) to deduce that such a sequence $u_n$ admits a strongly convergent subsequence in $L^2$ by virtue of the compactness of the embedding of $W^{1,2}$ into $L^2$, however this still does not assure us that the strong limit belongs to $W^{1,2}$.
And even if it did, we would then only be able to conclude that $u$ and its gradient are  also approximated by smooth functions in the $H^1$ norm using the Meyers-Serrin ``$H=W$" theorem.
...
In any event, proposition \ref{prop:Zhikov5-5} is not to be believed without concrete justification, although the result \cite[lemma 6.1]{zhikov2002homogenization} can be salvaged using the style of argument that we employ.
