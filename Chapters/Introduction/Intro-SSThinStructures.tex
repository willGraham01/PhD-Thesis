...
However, the studies \cite{kuchment2003asymptotics, exner2005convergence} later demonstrated that this was not the complete story.
...
\begin{itemize}
	...
	The spectral problem for the resulting operator $\mathcal{G}$ can be shown to define a quantum graph problem whose vertex conditions are continuity at each of the vertices, but along with a ``non-standard" Kirchoff condition at the vertices, \tstk{Wentzell is used in Post's monograph}
	\begin{align*}
	\sum_{v_k \text{ connects to } v_j} 
...
More precisely, the operator $\mathcal{G}$ acts in an \emph{exteneded} (or ``larger") Hilbert space $\mathcal{H}^+$, rather than a ``standard" space $\mathcal{H}\subset\mathcal{H}^+$ of functions on metric graphs, such as those defined in \eqref{eq:GraphFuncSpaces}.
...
\tstk{There is a physical interpretation that the original, dissipative problem $\mathcal{G}_K$ is extended to a conservative system $\mathcal{G}$ by adding in ``hidden" degrees of freedom, and conversely that $\mathcal{G}_K$ is a particular restriction of $\mathcal{G}$ - refs for this are? Figotin-Schenker?}
Further details and information on generalised resolvents and extensions of symmetric operators can be found in \cite{strauss1999function} --- we will not delve further into the available theory on this topic, although we shall see in \tstk{derivation parts, and talk about how our Sob spaces are the extended spaces} our approach via singular structures provides a natural construction of these extended spaces.
