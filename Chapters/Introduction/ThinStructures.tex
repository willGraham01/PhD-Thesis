...
\end{align*}
and write the resulting system in matrix form as
\begin{align*}
	-\recip{\rmi\epsilon_m}\curl{}H = \omega E, 
	&\qquad \grad\cdot \epsilon_m \mathbf{E} = 0,
	\recip{\rmi\mu_m}\curl{}E & \omega H, 
	&\qquad \grad\cdot \mu_m \mathbf{H} = 0.
\end{align*}
...
\subsection{Non-dimensionalisation of the Maxwell System}
...
\end{align*}
and we also suppose that the inclusions have a characteristic size $l\delta$, $0<\delta<1$, giving the period cell illustrated in figure \ref{fig:Diagram_ScalingDimensionfull}.
\begin{figure}[t]
	\centering
...
\begin{align*}
	...
	\qquad x = l\tilde{x},
\end{align*}
where $\hat{h}$ is a constant with the units of magnetic field ($\mathrm{A}\mathrm{m}^{-1}$), and $\epsFS$ and $\muFS$ are (respectively) the permittivity and permeability of vacuum.
The Maxwell system
\begin{align*}
...
\end{align*}
the noting that the frequency in the bulk $\omega_{\mathrm{bulk}}$ and wavelength in the bulk $\lambda_{\mathrm{bulk}}$ are related by 
\begin{align*}
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
for $\tilde{x}\in\reals^d$, with a period cell $\widetilde{Q} = \recip{l}Q = [0,1)^d$, an inclusion $\widetilde{Q}_0 = \recip{l}Q_0$ of size $\delta$, a bulk region $\widetilde{Q}_1 = \recip{l}Q_1$, and (dimensionless) relative permittivity and permeability
\begin{align*}
	\eps_r\bracs{\tilde{x}} = 
	\begin{cases} 
	...
	\end{cases}
\end{align*}
From here, one can of course derive the dimensionless ``curl-of-the-curl" equations and acoustic approximations.

The non-dimensionalisation process provides us with a problem that possesses three independent \emph{dimensionless} parameters,
\begin{align*}
	\delta, \qquad
...
Studying the Maxwell system and the (non-dimensional) spectrum $z = \frac{\omega}{\omega_{\mathrm{bulk}}} \frac{l}{\lambda_{\mathrm{bulk}}}$ wr
Importantly, all of these independent parameters refer to contrasts between physical properties, rather than being physical properties themselves.
This distinction is often not highlighted (or even acknowledged) in the mathematical literature \tstk{notable exceptions including K-Serena, and their reference?}, where the non-dimensional Maxwell system \eqref{eq:Intro-NonDimMaxwell} is simply introduced as \emph{the} Maxwell system.
The parameters $\eps, \mu$ are also introduced under the names electric permittivity and magnetic permeability --- rather than as the \emph{contrasts} between these physical properties.
