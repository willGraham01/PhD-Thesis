...
Having provided an outline of the important governing equations and operators that describe wave propagation in photonic crystals, we move on to discussing how their physical structure is represented.
...
We will go into more details in section \tstk{which section?!}, but the net effect of this transform is that we can replace the spectral problem for $\mathcal{A}$ with the spectral problem for each member of a family of operators $\mathcal{A}_{\qm}$ parametrised by the quasi-momentum.
...
\tstk{maybe new section for Photonic crystal domain setup? IDK, but you need some kind of linking sentence here}
...
\end{align*}
in the unit cell.
Finally, suppose that the inclusion $Q_0$ is of size $\delta<1$, so fills a volume of the order $\delta^d$ \tstk{make figure}.
...
Regrettably this process of non-dimensionalising is often skipped over in the mathematics literature, which has implications for the resulting studies which we highlight \tstk{when we talk about missing length scales, EP/KZ, etc}.
