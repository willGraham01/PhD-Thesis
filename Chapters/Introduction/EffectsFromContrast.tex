...
In this section, we put the shoe on the other foot so to speak, and focus on the various behaviours that can emerge when considering asymptotic limits of the material properties, or ratio of wavelengths in the various components of the medium.
In this section, the studies we discuss typically concern periodic, composite materials whose domains are described in the manner introduced in \tstk{section for composite materials}.
Throughout, we will phrase our discussion in terms of the values of the non-dimensional length parameters \tstk{refer to equation for this!} to avoid confusion where possible, and treat the composite as having material parameters described by the (possibly matrix-valued) $Q$-periodic function $A(y)$.
