...
\end{itemize}
\tstk{note that KZ missed out the extra cases, likely because they skipped over the proper non-dim process?}
...
To conclude our discussion, we highlight that at present there are no results in the literature which extend the studies \cite{kuchment2001convergence, exner2005convergence} into the Maxwell setting (that is, to either the curl-of-the-curl equation or the full Maxwell system).
...
Akin to \cite{kuchment2001convergence}, these studies do not account for the possible ``borderline" scaling case, with only the edge thickness $\delta$ being studied --- for this reason it is likely that results similar to those presented by \cite{exner2005convergence} exist within the context of elasticity too, however this remains an open problem.