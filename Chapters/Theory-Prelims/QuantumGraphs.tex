...
Also assign each vertex $v_j$ a coupling constant $\alpha_j\in\complex$ and\footnote{We will shortly restrict ourselves to the physically meaningful $\alpha\in\reals_{+}$, see \tstk{where our modelling assumptions are laid out}.} denote by $\alpha$ the diagonal matrix with $\alpha_1,...,\alpha_{\abs{\vertSet}}$ on the diagonal.
...
\end{assumption}
Note that we will tend to use $\ddom$ as the standard notation for the period cell, and $\graph$ for the period graph from this point forwards. \tstk{then why not just change all the $Q$s to $\ddom$s in the introduction too, Will?}
...
Restricting ourselves to the case of straight edges between vertices may seem overly zealous, however we will comment on the (rather straightforward) generalisation to non-straight edges in chapter \tstk{ensure that you DO put this in in chapter 3, after the derivation of the QG equations}.

\subsection{Function spaces} \label{ssec:QG-FunctionSpaces}
Function spaces on metric graphs are formed from taking the direct sum of function spaces on each of the edges; consider for example the following function spaces: \tstk{slight inconvenience here... you haven't defined notation for $L^2$ and $H^1$ spaces yet...!}
...
\begin{align*}
	...
	\end{cases}
\end{align*}
\tstk{commented out the generalised resolvents stuff --- this should go in the scalar chapter when we first derive our QG system with the spectral parameter on the RHS of the boundary condition.}
...
We next provide a brief overview of the $M$-matrix, a tool for characterising the spectrum of a quantum graph which we will exploit in section \tstk{other refs \ref{sec:ScalarDiscussion}}.
...
By choosing a different embedding, one could obtain a different set of fibres $\widetilde{\mathcal{A}}_{\qm}$ (and $M$-matrices), where in general $\sigma\bracs{\mathcal{A}_{\qm}} \neq \widetilde{\mathcal{A}}_{\qm}$, but the unions over the quasi-momentum still coinciding (with $\sigma{\mathcal{A}}$),
\begin{align*}
	\bigcup_{\qm\in B} \sigma\bracs{\mathcal{A}_{\qm}} &= \bigcup_{\qm\in B} \widetilde{\mathcal{A}}_{\qm} = \sigma{\mathcal{A}}.
\end{align*}
