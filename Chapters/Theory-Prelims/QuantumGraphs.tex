...
As was discussed in section \tstk{lit review, relevant parts \ref{sec:}} we shall be representing singular structures as graphs embedded into real space.
...
Our choice to embed (metric) graphs into $\reals^2$ is both a convenient way to represent the singular structures we want to examine (see section \ref{ssec:SingularStructures}), and allows us to define what it means for a graph to be ``periodic" rather intuitively.
...
We use similar notation for the when we consider a differential operator that has been shifted due to an application of the Gelfand transform \tstk{HAVE YOU INTRODUCED GELFAND YET?},
...
\tstk{this needs to come after we've seen the Gelfand transform}
