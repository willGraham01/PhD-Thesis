...
\end{assumption}
\tstk{check assumptions are consistent throughout the work!}
...
\begin{subequations} \label{eq:GraphFuncSpaces}
	...
	\end{align}
\end{subequations}
We define $u^{(jk)} = u\vert_{I_{jk}}$ to be the restriction of a function $u$ defined on $\graph$ to the edge $I_{jk}$, extended by zero to the other edges of the graph. \tstk{this will have repercussions in the scalar chapter! might need one convention for scalar- and one for vector valued functions}
