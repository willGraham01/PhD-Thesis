...
One can demonstrate (using density arguments) that $\gelfand$ can be extended to an isometry from $L^2\bracs{\reals^d}$ to $L^2\bracs{\ddom}$-valued functions on $B$. \tstk{kuchment review, section 7.3 and references therein}.

Now consider a differential operator $\mathcal{A} = \mathcal{A}\bracs{\grad}$ with $\ddom$-periodic coefficients acting on functions defined in $\reals^d$, for example the acoustic approximation operator $-\grad\cdot a(x)\grad u$ for some $\ddom$-periodic $a(x)$.
Through a short (and formal) computation, we can deduce that
\begin{align*}
	\sqbracs{\gelfand\bracs{\mathcal{A}\bracs{\grad_x}u}}(x,\qm) 
	&= \mathcal{A}\bracs{\grad_x + \rmi\qm}\gelfand u\bracs{x,\qm},
\end{align*}
where we have used the subscript $x$ to indicate that the gradient operator $\grad$ still acts with respect to the variable $x$.
