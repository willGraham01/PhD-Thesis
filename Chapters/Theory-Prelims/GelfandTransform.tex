...
\end{align}
for any $m'\in\integers^d$, so provided we know the values of the function $\gelfand u$ on $Q\times B$, we can reconstruct $\gelfand u$ for any $x\in\reals^d$.
For each $u\in\csmooth{\reals^d}$ and $\qm\in B$, the function $\gelfand u\bracs{\cdot,\qm}$ belongs to the space of smooth functions on the torus whose ``unravelling" \tstk{whats the correct terminology here?} into $\reals^d$ is $Q$ --- this new domain is compact, which is central to the utility of this transform.
