...
Since we will be working with a number of different measures as introduced in section \ref{sec:SingularMeasures}, the content of this section is presented with respect to a general Borel measure $\rho$ defined on (a subset of) $\reals^2$.
...
Let $\rho$ be a Borel measure on $\ddom$, and recall the shifted gradient ($\ktgrad$) and curl operators ($\ktcurl{}$) operators introduced in section \ref{sec:TP-GelfandTransform}.
...
\tstk{also need the whole ``periodic, but can ignore b/c graphs" lecture, although this might go in the first appendix to chapter 3}
...
We also use smooth periodic functions, and the $\ktgrad$ operator in place of the usual gradient operator to reflect our application of the Gelfand transform --- see section \ref{ssec:SobSpacesAndGelfand}.
...
Upon recalling section \ref{sec:TP-GelfandTransform}, the reader may noticed that our notation and construction of the Sobolev spaces $\ktgradSob{\ddom}{\rho}$ and $\ktcurlSob{\ddom}{\rho}$ is heavily suggestive of being the result of the combination of a Gelfand transform in the first two coordinate directions and a Fourier transform in the third.
...
\begin{align*}
	W_{\upsilon,\mathrm{grad}} := \overline{ \clbracs{ \bracs{\phi, \grad\phi} \setVert \phi\in\smooth{\reals^d} } },
\end{align*}
...
Consider the extrusion $\dddom:=\hat{\graph}\times[0,\infty)\subset\reals^3$ equipped with the product measure $\upsilon\times\lambda_1$, and the image of the set
\begin{align*}
	\clbracs{ \bracs{\phi, \grad\phi} \setVert \phi\in\smooth{\dddom} }
\end{align*}
...
\end{align*}
By virtue of how we chose to construct the space $\gradSob{\dddom}{\bracs{\upsilon\times\lambda_1}}$, we have that the image of a function $u\in\gradSob{\dddom}{\bracs{\upsilon\times\lambda_1}}$ will be some $\hat{u}\in\ktgradSob{\ddom}{\rho}$ where $\rho$ is the restriction of the measure $\upsilon$ to the period cell $\ddom$.
Similar conclusions can be drawn for the spaces $\curlSob{\dddom}{\bracs{\upsilon\times\lambda_1}}$ and $\ktcurlSob{\ddom}{\rho}$ and, if one does not consider an extrusion (hence nor a Fourier transform), the spaces $\gradSob{\reals^2}{\upsilon}$ and $\tgradSob{\ddom}{\rho}$.
Since the Gelfand and Fourier transforms are isometries, we can also make the reverse associations.