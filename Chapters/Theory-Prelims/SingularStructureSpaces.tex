...
The idea is to construct an analogy to a Sobolev space for the measure $\rho$, and hence obtain a concept of (weak) derivative. \tstk{this sentence might be more useful elsewhere, but the following sentence does depend on it}
...
\end{proof}
Proposition \ref{prop:ZeroInvariantUnderQM-Wavenumber} will be helpful later \tstk{section refs} when we want to understand the form/ behaviour of gradients (and curls) of zero for the measures $\dddmes, \ddmes$, and $\nu$.
...
\tstk{you haven't defined divergence-free yet????}
